...
float brightness(vec3 p) {
   float n = max(fbn(vec4(p * 0.1, time)), 0.0);
   float spots = max(snoise(vec4(p * 0.02, time )), 0.0);
   float b = mix(1.0, spots, 0.7) * n;
   return b;
}
...
float fresnel(vec3 eye, vec3 n) {
   return pow( 1.0 - dot(eye, n), 0.8);
}

float fresnel2(vec3 eye, vec3 n) {
   return pow( dot(eye, n), 0.8) * 1.2;
}
